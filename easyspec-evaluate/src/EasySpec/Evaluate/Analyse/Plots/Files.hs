...
pointsPlotForEvaluators
    :: MonadIO m
    => Evaluator -> Evaluator -> m (Path Abs File)
...
singleEvaluatorBarPlotFileForExampleAndName
    :: MonadIO m
    => ES.InputSpec -> ES.EasyName -> Evaluator -> m (Path Abs File)
...
singleEvaluatorBarAnalysisScript
    :: MonadIO m
    => m (Path Abs File)
