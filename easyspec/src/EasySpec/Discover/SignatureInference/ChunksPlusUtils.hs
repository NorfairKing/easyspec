module EasySpec.Discover.SignatureInference.ChunksPlusUtils where
...
inferChunksPlusFrom ::
    ...
    -> [EasyId]
    -> InferredSignature
inferChunksPlusFrom shrink = inferShrinkThenDrill shrink drillChunksPlus

drillChunksPlus :: [EasyId] -> [EasyId] -> InferredSignature
drillChunksPlus focus scope =
    InferredSignature $ do
        ...
        level2Tups <-
            forM tups $ \(ff, otherFunc) ->
                ...
                in (,,) (ff, otherFunc) nexs <$> inferFromWith nexs [level1t]
        let trips = [(a, b, c) | a <- focus, b <- scope, c <- scope, b < c]
        forM_ trips $ \(ff, sf1, sf2) -> do
            let mentionedNodes sf_ =
                    filter
                        (\((sf, other), _, _) -> sf == ff && other == sf_)
                        level2Tups
                          -- The nodes that mention ff and sf1
                dependencies = mentionedNodes sf1 ++ mentionedNodes sf2
                ...
                foundSomething (_, nexs, (_, eqs)) =
                    flip all nexs $ \nex ->
                        flip any eqs $ \eq -> mentionsEq (neName nex) eq
            when (all foundSomething dependencies) $ do
                let ns = nub $ concatMap (\(_, nexs, _) -> nexs) dependencies
